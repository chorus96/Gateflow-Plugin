...
**하드웨어를 사랑하는 데 진입 장벽이 있어서는 안 됩니다.**

GateFlow는 전문적인 하드웨어 개발 도구를 Claude Code로 가져옵니다. 만들고 싶은 것을 — 목표 보드까지 포함해 — 설명하면, 올바른 핀 할당과 함께 lint 검사, 시뮬레이션, 정형 검증, 합성 준비가 완료된 코드를 얻을 수 있습니다.

첫 `always_ff`를 작성하든, FIFO가 절대 오버플로하지 않음을 정형적으로 증명하든, Arty A7용 SPI 컨트롤러를 합성하든, 도구는 당신과 싸우는 것이 아니라 당신을 도와야 합니다.
