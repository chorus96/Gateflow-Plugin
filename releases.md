...
## 2.0.0 (2026-03-26) — Formal 검증 + 합성 + IP 라이브러리

### 신규 기능
- **자연어 기반 Formal 검증** (`/gf-formal`): 증명할 것을 영어로 설명하면 SVA 프로퍼티 + SymbiYosys 증명을 얻습니다. 킬러 기능.
- **Yosys 합성** (`/gf-synth`): 면적/타이밍 리포트(LUT, FF, BRAM, DSP)와 함께 설계를 합성. 실패하기 전에 지원되지 않는 SV 구문을 경고.
- **IP 블록 라이브러리** (`/gf-ip`): 바로 쓰는 검증된 하드웨어 컴포넌트 8개 — 각각 RTL, 자가 검사 테스트벤치, Formal 프로퍼티, 문서 포함.
...
### 신규 에이전트
- `sv-formal` — Formal 검증 전문가 (SVA + SymbiYosys)
- `sv-synth` — 합성 최적화 전문가 (Yosys)

### 신규 커맨드
- `/gf-formal` — Formal 검증 실행
